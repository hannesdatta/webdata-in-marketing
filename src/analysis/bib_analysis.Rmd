...
```{r, echo = FALSE}
data = data.frame(S$MostRelKeywords[,1:2])
colnames(data) <- c('keywords','articles')
data$articles <- as.numeric(data$articles)
data$keywords <- as.character(data$keywords)

data$keywords <- factor(data$keywords, levels=unique(data$keywords))

ggplot(data[1:20,], aes(x = keywords, y = articles, fill = keywords)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label = as.character(keywords)), vjust = 0)
