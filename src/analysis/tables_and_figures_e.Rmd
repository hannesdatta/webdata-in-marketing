...
for (i in 1:18) {
  cites_yearly[, paste0('years_since_', i):= (year-pubyear+1)==i & web == F & journal == "JCR"]
  cites_yearly[, paste0('years_websince_', i):= (year-pubyear+1)==i & web == T  & journal == "JCR"]
}
...
extracted_coefs %>% filter(year<=15) %>% 
  ...
  geom_errorbar(aes(ymin=Estimate-1.69*`Std. Error`, ymax=Estimate+1.69*`Std. Error`), width=.2,
                position=position_dodge(0.05)) + xlab('Years Since Publication') + ylab('Total Number of Non_Marketing Citations')+scale_fill_manual(values = c("gray30", "grey85", "gray60", "gray40", "gray1")) +  theme_bw() + guides(linetype = guide_legend(reverse=F)) + 
...
ggsave('../../gen/analysis/output/JCR_citations.png',width=6,height=5)
